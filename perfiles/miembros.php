...
<table class="table table-striped" >
  ...
  for ($i=0; $i < $miconexion->numregistros(); $i++) { 
    $miembro=$miconexion->consulta_lista();
      echo "<tr>";
      if ($miembro[4]==""){
        if ($miembro[9]=="Femenino") {
        ...
        echo "<td style='width:40px;'><img class='img-circle' style='width:40px; height:40px;' src='images/".$miembro[10]."/".$miembro[4]."'></td>";
      }
      if ($miembro[3]==$miembro[5]) {
        echo  "<td style='font-size: 9px;'><span style='font-size: 11px; color: #006064; font-weight: bold;'>".strtoupper($miembro[1]." ".$miembro[2])."</span> <strong>(Administrador)</strong><br>".$miembro[7]."<br> Miembro desde ".date('d-m-Y',strtotime($miembro[8]))."</td>";
        echo "<td style='width:19.43px;'></td>";
      }else{
      if ($miembro[6]=='solicitud') {
        echo  "<td style='font-size: 9px;'><span style='font-size: 11px; color: #006064; font-weight: bold;'>".strtoupper($miembro[1]." ".$miembro[2])."</span><br>".$miembro[7]." (Invitado)<br> Invitado el ".date('d-m-Y',strtotime($miembro[8]))."</td>";
        echo "<td style='width:19.43px;'></td>";
      }else{
        if ($miembro[5]==$_SESSION['id']){ 
        echo  "<td style='font-size: 9px;'><span style='font-size: 11px; color: #006064; font-weight: bold;'>".strtoupper($miembro[1]." ".$miembro[2])."</span><br>".$miembro[7]."<br> Miembro desde ".date('d-m-Y',strtotime($miembro[8]))."</td>";
        echo '<td class="btn-group pull-right" style="padding-left:0px; padding-right:10px;">
            ...
            <ul class="dropdown-menu pull-right" role="menu">
              <li>
                <a onclick="actualizar_notificacion('8','<?php echo $id ?>','<?php echo $miembro[3] ?>')" style="width:100%; display:inline-block; font-size:11px;" class="btn btn-default">
                  Nombrar Administrador
                </a>
              </li>
              <li>
                <a onclick="actualizar_notificacion('7','<?php echo $id ?>','<?php echo $miembro[3] ?>')"  style="width:100%; display:inline-block; font-size:11px;" class="btn btn-default">
                  Eliminar del grupo
                </a>
              </li>
            </ul>
          ...
          echo "<td style='font-size: 9px;'><span style='font-size: 11px; color: #006064; font-weight: bold;'>".strtoupper($miembro[1]." ".$miembro[2])."</span><br>".$miembro[7]."<br> Miembro desde ".date('d-m-Y',strtotime($miembro[8]))."</td>";
          echo "<td style='widt`h:19.43px;'></td>";
        }
      }
    }
      echo "</tr>";
