<?php
global $notifi;
$miconexion->consulta("select g.nombre_grupo, p.nombre_partido, p.fecha_partido, p.hora_partido , cd.centro_deportivo, n.id_noti, n.id_partido, p.id_user 
...
for ($i=0; $i < @$miconexion->numregistros(); $i++) {
  ...
  if ($hora_comp <= date('Y-m-d H:i:s', $hora_p)) {
    ...
    <span class='label label-sm label-icon label-success' onclick='actualizar_notificacion("4","<?php echo $notifi[6] ?>", "<?php echo $notifi[5] ?>" ,"", "<?php echo $notifi[7] ?>"  );'>
    <i class='icon-ok'></i>
    </span>
