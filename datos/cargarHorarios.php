<?php 
	...
	$miconexion->consulta("select p.nombre_partido, p.fecha_partido, p.hora_partido, cd.tiempo_alquiler, p.estado_partido from partidos p, centros_deportivos cd where p.id_centro = cd.id_centro and p.id_centro = '".$_POST['centro']."' order by p.fecha_partido");
	for ($i=0; $i < $miconexion->numregistros(); $i++) { 
